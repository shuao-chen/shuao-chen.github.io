...
Under the transformation $(\tilde Y_i, T_i) \mapsto \left(\tilde Y_i,\; \frac{T_i}{f_{Y|X}(\tilde Y_i|x)}\right)$, the time coordinate is rescaled, and the intensity measure becomes $f_{Y|X}(y|x)\,dy \times dt$.
$\blacklozenge$
...
The first step uses the continuity of exponential random variables, so ties occur with probability zero. The second step conditions on the value of $E_i$ and applies the law of total probability. The third step uses the independence of $\{E_j\}$ to factorize the joint probability. For the fourth step, since $E_j \sim \mathrm{Exp}(\lambda_j)$, its density is $f_{E_j}(t)=\lambda_j e^{-\lambda_j t}$ for $t\ge 0$, and hence $\mathbb P(E_j > t) = \int_t^\infty \lambda_j e^{-\lambda_j s}\,ds = e^{-\lambda_j t}$. The fifth step combines the exponential terms, and the last step evaluates the resulting integral using $\int_0^\infty e^{-at}dt=1/a$.
$\blacksquare$
...
Next, we show that this construction indeed generates the target conditional distribution. Since $\frac{Z_y}{p_{Y|X}(y|x)} \sim \mathrm{Exp}\big(p_{Y|X}(y|x)\big)$, and the variables corresponding to different $y$ are independent, by Lemma 2 (the minimum property of exponential distributions), we have
$$\begin{align}
...
Moreover, it suffices to record $y\in\\{1,\dots, \|\mathcal Y\|-1\\}$, since for each fixed $x$, the probabilities satisfy the normalization condition $\sum_{y\in\mathcal Y} p(x,y \mid z)=p(x \mid z)$, so the last component is uniquely determined by the others.

From the above representation, the vector $\Big(H(Y \| Z),\;p(x,y):x\in\mathcal X,\; y\in\\{1,\dots,\|\mathcal Y\|-1\\}\Big)$ can be expressed as a convex combination of $\\{v(z):z\in\mathcal Z\\}$, i.e., it lies in their convex hull.
