...
\begin{align}
    ...
    &= H(Y|Z),
\end{align}
where the second step uses $Z \perp\!\!\!\perp X$, and the third step uses $Y=g(X,Z)$.
...
\begin{align}
    ...
    \qquad
    T_i - T_{i-1} \overset{\text{i.i.d.}}{\sim} \mathrm{Exp}(1), \quad T_0=0.
\end{align}
...
This selection rule assigns a weight to each point $(\tilde Y_i,T_i)$, where the weight is given by the likelihood ratio $\frac{dP_Y}{dP_{Y|X}}(\tilde Y_i|x)$, thereby introducing a non-uniform scaling in the time coordinate depending on $x$.
...
In this case, the marked point process in the Poisson functional representation is \(Z=\{(\tilde Y_i,T_i)\}_{i=1,2,\dots},\) where $\tilde Y_i \overset{\text{i.i.d.}}{\sim} \mathrm{Unif}[0,1]$, and $\{T_i\}$ are the arrival times of a Poisson process with rate $1$, i.e.,
\begin{align}
    T_i-T_{i-1}\overset{\text{i.i.d.}}{\sim}\mathrm{Exp}(1),\quad T_0=0.
\end{align}
...
From a geometric perspective, the point set $\{(\tilde y_i,t_i)\}$ can be viewed as a collection of random points on $[0,1]\times\mathbb{R}_+$, where the horizontal coordinate $\tilde y_i$ is uniformly distributed over $[0,1]$, and the vertical coordinate $t_i$ represents increasing random times. For each point $(\tilde y_i,t_i)$, we compare its vertical coordinate with the value of $f_{Y|X}(y|x)$ at the corresponding position, thereby forming a normalized ratio and selecting the point with the smallest value.

Equivalently, from a dynamic perspective, one can imagine gradually scaling up the graph of $f_{Y|X}(y|x)$; when it first touches a random point, the horizontal coordinate of that point is taken as the output.
...
\begin{align}
    ...
    =e^{-\lambda t}.
\end{align}
This is exactly the tail distribution of an exponential distribution with parameter $\lambda$, hence \(\tau \sim \mathrm{Exp}(\lambda).\)
\end{IEEEproof}
...
so that
\begin{align}
    \tau_A \sim \mathrm{Exp}\!\left(\int_A f_{Y|X}(y|x)\,dy\right).
\end{align}

Similarly,
\begin{align}
    \tau_{A^c} \sim \mathrm{Exp}\!\left(\int_{A^c} f_{Y|X}(y|x)\,dy\right),
\end{align}
...
\begin{align}
    T_i \sim \mathrm{Exp}(\lambda_i),\qquad \lambda_i>0,\; i=1,\dots,n.
\end{align}
Then
\begin{align}
...
In summary, this construction rescales the time coordinate depending on $x$, transforming the distribution of the mark of the earliest arrival point from the uniform distribution to the target conditional distribution. Since the point set $\{(\tilde Y_i, T_i)\}$ is randomly generated (with both $\tilde Y_i$ and $T_i$ being random variables), by repeatedly generating the process independently and applying the above selection rule, one obtains independent samples following $f_{Y|X}(\cdot|x)$.

### Discrete Case: Exponential Race Representation

When $\mathcal{Y}$ is a finite set, the above Poisson construction can be equivalently represented as an exponential race form. Let $\{Z_y\}_{y\in\mathcal{Y}}$ be a collection of independent exponential random variables,
\begin{align}
    Z_y \sim \mathrm{Exp}(1), \; y\in\mathcal{Y}.
\end{align}
...
This construction can be interpreted as a “competition” among all candidate values $y$: each $y$ is associated with a random time $Z_y$, which is scaled according to the weight $p_{Y|X}(y|x)$, and the one with the smallest scaled value is selected.
...
\begin{align}
    \min_{i:\tilde Y_i=y} T_i \sim \mathrm{Exp}\big(p_Y(y)\big),
\end{align}
...
Next, we show that this construction indeed generates the target conditional distribution. Since
\begin{align}
    \frac{Z_y}{p_{Y|X}(y|x)} \sim \mathrm{Exp}\big(p_{Y|X}(y|x)\big),
\end{align}
and the variables corresponding to different $y$ are independent, by the minimum property of exponential distributions, we have
\begin{align}
...
Moreover, the selection rule is invariant to common scaling of $\{Z_y\}$, i.e., for any constant $c>0$,
\begin{align}
    \arg\min_{y} \frac{Z_y}{p_{Y|X}(y|x)}
    =
    \arg\min_{y} \frac{cZ_y}{p_{Y|X}(y|x)}.
\end{align}

Hence, one can normalize $\{Z_y\}$ such that
\begin{align}
    \sum_{y\in\mathcal{Y}} Z_y = 1,
\end{align}
in which case the random vector $\{Z_y\}_{y\in\mathcal{Y}}$ is uniformly distributed over the probability simplex, i.e., follows a Dirichlet$(1,\dots,1)$ distribution. This normalization does not affect the selection result.
...
When $P_{Y|X}(\cdot|x)$ is close to $P_Y$, we have
\begin{align}
    \frac{dP_Y}{dP_{Y|X}(\cdot|x)}(y)\approx 1,
\end{align}
in which case the selection rule is mainly determined by $\{T_i\}$. Since $\{T_i\}$ is increasing, smaller indices $i$ are more likely to be selected, hence $K$ tends to be small; in particular, when $P_{Y|X}(\cdot|x)=P_Y$, we have $K=1$.

This shows that the uncertainty of $K$ is closely related to the discrepancy between $P_{Y|X}(\cdot|x)$ and $P_Y$. One can show
\begin{align}
    \mathbb{E}[\log K \mid X=x]
    \le
    D\big(P_{Y|X}(\cdot|x)\,\|\,P_Y\big)
    + e^{-1}\log e + 1.
\end{align}
...
\begin{align}
    \mathcal{L}
    =
    -\sum_{k=1}^\infty p_K(k)\log p_K(k)
    + \alpha\Big(\sum_{k=1}^\infty p_K(k)-1\Big)
    + \beta\Big(\sum_{k=1}^\infty p_K(k)\log k - L\Big).
\end{align}
...
hence
\begin{align}
    H(K)\le -\sum_{k=1}^\infty p_K(k)\log q(k).
\end{align}
...
Next, estimate $c$. Note that
\begin{align}
    c^{-1}=\sum_{k=1}^\infty k^{-\lambda}.
\end{align}
...
\begin{lemma}[Independent Restriction Property of Poisson Point Processes]
...
Taking expectation over $(\Theta,Y)$, and using $\Theta\sim \mathrm{Exp}(1)$ and $Y\sim P_{Y|X}(\cdot|x)$, we obtain
\begin{align}
    \mathbb E[\log K\mid X=x]
    &=
...
From the above representation, the vector
\begin{align}
    \Big(H(Y|Z),\;p(x,y):x\in\mathcal X,\; y\in\{1,\dots,|\mathcal Y|-1\}\Big)
\end{align}
can be expressed as a convex combination of $\{v(z):z\in\mathcal Z\}$, i.e., it lies in their convex hull.
