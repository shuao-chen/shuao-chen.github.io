...
We now show that the above Poisson functional representation construction can generate the target conditional distribution \(Y \mid \{X=x\} \sim P_{Y|X}(\cdot|x).\)

Fix any $x$, and assume that \(P_{Y|X}(\cdot|x) \ll P_Y\). Consider the marked Poisson point process \(Z = \{(\tilde Y_i, T_i)\}_{i\ge 1},\) where $\tilde Y_i \overset{\text{i.i.d.}}{\sim} P_Y$, and $\{T_i\}$ are the arrival times of a Poisson process with rate $1$. The intensity measure of this point process on $\mathcal Y\times\mathbb R_+$ is
\begin{align}
    \nu(dy,dt)=P_Y(dy)\,dt.
\end{align}
...
Applying this lemma to the current setting with $P=P_{Y|X}(\cdot|x)$, we obtain
\begin{align}
    Y_K' \sim P_{Y|X}(\cdot|x).
\end{align}
